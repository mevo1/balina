...
function saveBot() {
    ...
    const api_id = selectedApi ? selectedApi.id : null; // API IDSİ
    ...
    fetch(url, {
            ...
            body: JSON.stringify({ name, strategy_id, coin_list, interval, first_amount, moving_tp, moving_sl, api_id })
    })
    .then(response => {
...
function fillingBot(id){
    fetch(`/boting/api/bot-view/${id}/`)
            ...
            .then(data => {
                ...
                // API ID
                const apiRadio = document.getElementById(data.api_id);
                if (apiRadio) {
                    apiRadio.checked = true; // API radio butonunu seçili yap
                }
